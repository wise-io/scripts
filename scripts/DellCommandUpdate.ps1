...
function Get-DellCommandUpdate {
  ...
  $Apps = $ModelXMLContent.Manifest.SoftwareComponent | Where-Object {
    ...
    $_.SupportedOperatingSystems.OperatingSystem.osArch -match $SystemArch
  }
  
  $Latest = $Apps | Sort-Object -Property 'vendorVersion' | Select-Object -Last 1
  if ($Latest) {
    return @{
...
else { Write-Output "`nIf updates were installed, a reboot may be needed to complete the installation." }
